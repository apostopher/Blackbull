            ...
            if (scrips.length > 0) {
                $.getJSON("/serverscripts/portfolio/getLTP.php?user=" + user_id + "&scrips=" + scrips, function (jsonLTP) {
                    var count = 0,
                        ...
                        market_qty = parseInt(buy_qty, 10) - parseInt(sell_qty, 10);
                    for (count = 0; count < jsonLTP.total; count += 1) {
                        ...
                        unret = market_qty * (parseFloat(jsonLTP.data[count].ltp) - parseFloat(avg_buy));
                ...
                tooltip: {
                    formatter: function () {
                        var toolcount = 0,
                            ...
                            returntxt;
                        if (this.point.id) {
                            toolarray = this.point.id.split("_");
                            returntxt = "Close: " + this.y + " Rs.<br/><br/>";
...
/* Check whether the CLOSE array date is smaller than trans date 
            * This means user added a transaction on a holiday. :-) so we need to fix curr_date */
            ...
            transhtml = "";
            for (tcount = 0; tcount < jsontrades.transtotal; tcount += 1) {
            ...
            rreturnsstr = sprintf("%.1f", rreturns - (sellqty * avgbuy));
            ...
            urreturnsstr = sprintf("%.1f", (totalqty - sellqty) * (jsontrades.last - avgbuy));
            ...
            if (jsontrades.targetstotal > 0) {
                maxValue = 0;
                targethtml = "";
                for (count = 0; count < jsontrades.targetstotal; count += 1) {
                    arrayelement = [];
            ...
            if (jsontrades.sltotal > 0) {
                minValue = 0;
                slhtml = "";
                for (count = 0; count < jsontrades.sltotal; count += 1) {
                    if (count === 0) {
            ...
            $("#waitscreen").hide();
            $("#chartholder").show();
            chartfd = new Highcharts.Chart(closeoptions);
        } else {
