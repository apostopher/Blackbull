...
(function($){
  ...
  $.getJSON("/serverscripts/portfolio/getTrades.php?portfolio="+portfolio_id+"&user="+user_id,function(jsontrades){
	  ...
	  */
      ...
      /* Do some extra hover effects (optional) */
      ...
      $.ajax({
          ...
          success: function(response){
		      ...
		      datatrid = response.trade_id;
                      var new_row = "<div class=\"bodydiv clearfix\" id=\""+scripdivid+"\"><div class=\"first\"";
		  ...
		  $("#cancelbtn").trigger('click');
		  $("#resulttxt").html("Transaction added successfully!<br/><span class=\"undotxt\"><a id=\"undoa\" data-trsid=\""+response.trans_id+"\" data-trid=\""+datatrid+
		                       "\" data-avgbuy=\""+response.old_avg_buy+"\" data-qty=\""+response.old_qty+"\" href=\"javascript:void(0);\">Click here</a> to undo.</span>");
                  $("#undoa").click(function(event){
                    event.preventDefault();
                    var old_qty = $(this).attr("data-qty");
                    var old_avg = $(this).attr("data-avgbuy");
                    /* Make another request to delete the newly added transaction. */
                    $.ajax({
                      type: "POST",
                      url: "/serverscripts/portfolio/undoTransaction.php",
                      data:({"trans_id":$(this).attr("data-trsid"),
                             "trade_id":$(this).attr("data-trid"),
                             "old_avg":old_avg,
                             "old_qty":old_qty
                            }),
                      beforeSend: function(){requestInProcess = true;},
                      success:function(response){
                        if(response.status == "1"){
                          ...
                          $("#resulttxt").html("Undo succeeded&hellip;");
                          /* update the average buy and quantity*/
                          var editrow = "#"+scripdivid;
                      ...
                      error:function(){requestInProcess = false;}
                    });
                  });		  
              }else{
                  if(!$("#resulttxt").hasClass("red")){
        ...
        if(response.status == "1"){
          ...
          $("#undocasha").click(function(event){
            ...
            $.ajax({
              ...
              data:({"cash_id":$(this).attr("data-trid")}),
              beforeSend: function(){cashrequestInProcess = true;},
              success:function(response){
                if(response.status == "1"){
              ...
              error:function(){cashrequestInProcess = false;}
            });       
          });
          totalcash = parseFloat($("#cashvalue").html()) + response.cash;
          $("#cashvalue").html(sprintf("%.1f", totalcash));
...
function getLTP(scripnames){
    ...
    $.getJSON("/serverscripts/portfolio/getLTP.php?user="+user_id+"&scrips="+scrips,function(jsonLTP){
        for(count=0; count < jsonLTP.total; count++){
            ...
            var perchange = ((parseFloat(scripltp) - parseFloat(scripavgbuy))*100)/parseFloat(scripavgbuy);
            ureturns += scripchg;
	...
	$.getJSON("/serverscripts/getindices.php",function(response){
		if(response.error == "0"){
		  if(parseFloat(response.values[0].change) < 0){
		    if(!$("#nifty .indexvalue").hasClass("red")){
		      $("#nifty .indexvalue").addClass("red");
		    }
		  }else{
		    if($("#nifty .indexvalue").hasClass("red")){
		      $("#nifty .indexvalue").removeClass("red");
		    }
		  }
		  ...
		  if(parseFloat(response.values[1].change) < 0){
		    if(!$("#sensex .indexvalue").hasClass("red")){
		      $("#sensex .indexvalue").addClass("red");
		    }
		  }else{
		    if($("#sensex .indexvalue").hasClass("red")){
		      $("#sensex .indexvalue").removeClass("red");
		    }
		  }
