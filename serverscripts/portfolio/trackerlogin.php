	...
	while($row = mysql_fetch_array($result)){
		...
		if(isset($_POST['remember'])){
			if($_POST['remember'] == "1"){
				...
				setcookie("user_id", $_SESSION['user_id'], time()+60*60*24*100, "/","blackbull.in");
		...
		$portfolio = 0;
		$portresult = mysql_query("SELECT portfolio_id FROM portfolio_portfolios WHERE user_id=".$row['user_id']);
		if($portresult){
