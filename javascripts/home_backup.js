	...
	$.getJSON("/serverscripts/getindices.php",function(response){
		if(response.error == "0"){
			if(parseFloat(response.values[0].change) < 0){
				if(!$("#nifty").hasClass("indexred")){
                			$("#nifty").addClass("indexred");
                		}
			}else{
				if($("#nifty").hasClass("indexred")){
                			$("#nifty").removeClass("indexred");
                		}
			}
			...
			if(parseFloat(response.values[1].change) < 0){
				if(!$("#sensex").hasClass("indexred")){
                			$("#sensex").addClass("indexred");
                		}
            		}else{
            			if($("#sensex").hasClass("indexred")){
                			$("#sensex").removeClass("indexred");
                		}
            		}
